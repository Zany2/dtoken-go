...
func TestRouteAccessHandlerOption(t *testing.T) {
	options := defaultAuthOptions()
	WithRouteAccessHandler(func(_ context.Context, _ *hertzapp.RequestContext, req *RouteAccessRequest) {
		req.AuthType = "tenant:"
...
func TestBeforeAuthHandlerNextAndExit(t *testing.T) {
	...
	nextCount := 0
	options := defaultAuthOptions()
	WithBeforeAuthHandler(func(_ context.Context, _ *hertzapp.RequestContext, req *AuthHandleRequest) {
		req.Next()
	})(options)
...
func TestFailFuncOption(t *testing.T) {
	...
	var gotErr error
	options := defaultAuthOptions()
	WithFailFunc(func(_ context.Context, _ *hertzapp.RequestContext, err error) {
		gotErr = err
	})(options)

	options.FailFunc(context.Background(), nil, wantErr)
