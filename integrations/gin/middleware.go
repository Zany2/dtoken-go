...
type RouteAccessHandler func(ctx context.Context, c *gin.Context, req *RouteAccessRequest)
...
func AccessMiddleware(ctx context.Context, opts ...AuthOption) gin.HandlerFunc {
	options := defaultAuthOptions()
	for _, opt := range opts {
		opt(options)
	}
