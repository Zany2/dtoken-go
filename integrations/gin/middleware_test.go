...
func TestRouteAccessHandlerOption(t *testing.T) {
	options := defaultAuthOptions()
	WithRouteAccessHandler(func(_ context.Context, _ *gin.Context, req *RouteAccessRequest) {
		req.AuthType = "tenant:"
...
func TestBeforeAuthHandlerNextAndExit(t *testing.T) {
	...
	nextCount := 0
	options := defaultAuthOptions()
	WithBeforeAuthHandler(func(_ context.Context, _ *gin.Context, req *AuthHandleRequest) {
		req.Next()
	})(options)
...
func TestFailFuncOption(t *testing.T) {
	...
	var gotErr error
	options := defaultAuthOptions()
	WithFailFunc(func(_ *gin.Context, err error) {
		gotErr = err
	})(options)
