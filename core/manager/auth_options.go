...
// LoginOptions describes one login operation LoginOptions 描述一次登录操。
type LoginOptions struct {
	...
	// Timeout overrides token timeout for this login Timeout 覆盖本次登录。token 有效。
	Timeout time.Duration `json:"timeout"`
	// ActiveTimeout overrides active timeout for this token ActiveTimeout 覆盖本次 token 的活跃超。
	ActiveTimeout time.Duration `json:"activeTimeout"`
	// Token uses a pre-created token value Token 使用预生。token 。
	...
	// IsConcurrent overrides concurrent login switch IsConcurrent 覆盖并发登录开。
	IsConcurrent *bool `json:"isConcurrent,omitempty"`
	// IsShare overrides shared token switch IsShare 覆盖共享 token 开。
	...
	// ReplacedLoginExitMode overrides non-concurrent strategy ReplacedLoginExitMode 覆盖非并发登录处理策。
...
const (
	...
	// TerminateActionReplace marks terminal as replaced TerminateActionReplace 标记终端被顶。
	TerminateActionReplace TerminateAction = "replace"
)

// TerminateOptions describes one terminal operation TerminateOptions 描述一次终端下线操。
...
func validateLoginPolicyOverrides(opts LoginOptions) error {
