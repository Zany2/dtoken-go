...
var (
	...
	// ErrKeyNotFound indicates storage key is missing or expired ErrKeyNotFound 表示存储键不存在或已过期。
...
var (
	...
	// ErrDeviceDisabled indicates disabled device ErrDeviceDisabled 表示设备已被禁用。
	ErrDeviceDisabled = errors.New("device disabled: this device is disabled for this account")
	// ErrDeviceNotDisabled indicates device not disabled ErrDeviceNotDisabled 表示设备未被禁用。
