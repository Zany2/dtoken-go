...
// AuthorizationCode authorization code information
// 授权码信息
...
// TokenRequest Unified token request structure
// 统一的令牌请求结构
...
// UserValidator Function type for validating user credentials
// 验证用户凭证的函数类型
...
// RegisterClient Registers an OAuth2 client
// 注册OAuth2客户端
...
// UnregisterClient Unregisters an OAuth2 client
// 注销OAuth2客户端
...
// ValidateAccessTokenAndGetInfo Validates access token and get info
// 验证访问令牌并获取信息
...
// RevokeToken Revokes access token and its refresh token
// 撤销访问令牌及其刷新令牌
...
// isValidScopes Checks if requested scopes are allowed for client
// 检查请求的权限范围是否被允许
...
// isValidGrantType Checks if grant type is allowed for client
// 检查授权类型是否被允许
...
// generateAccessToken Generates access token and refresh token
// 生成访问令牌和刷新令牌
