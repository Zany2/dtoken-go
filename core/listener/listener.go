...
// EventData contains information about a triggered event.
// EventData 事件数据，包含触发事件的相关信息。
...
// String returns a string representation of the event data.
// String 返回事件数据的字符串表示。
...
// Listener is the interface for event listeners.
// Listener 事件监听器接口。
type Listener interface {
	// OnEvent is called when an event is triggered.
	// OnEvent 当事件触发时调用。
	// The listener should not panic; any panic will be recovered by the event manager.
	// 监听器不应该 panic，任何 panic 都会被事件管理器恢复。
	OnEvent(data *EventData)
}

// ListenerFunc is a function adapter that implements the Listener interface.
// ListenerFunc 函数适配器，实现 Listener 接口。
type ListenerFunc func(data *EventData)

// OnEvent implements the Listener interface.
// OnEvent 实现 Listener 接口。
func (f ListenerFunc) OnEvent(data *EventData) {
	f(data)
}

// ListenerConfig holds configuration for a registered listener.
...
// EventFilter is a function that decides whether an event should be processed.
// EventFilter 事件过滤器，决定事件是否应该被处理。
type EventFilter func(data *EventData) bool

// EventStats contains statistics about event processing.
// EventStats 事件统计信息。
...
// Manager manages event listeners and dispatches events.
// Manager 事件管理器，管理监听器并分发事件。
type Manager struct {
	mu              sync.RWMutex
...
// SetPanicHandler sets a custom panic handler for listener errors.
// SetPanicHandler 设置自定义的 panic 处理器。
...
// ClearFilters removes all event filters.
// ClearFilters 清除所有事件过滤器。
...
// EnableStats enables event statistics collection.
// EnableStats 启用事件统计。
...
// GetStats returns a copy of event statistics.
// GetStats 获取事件统计信息副本。
...
// ResetStats resets event statistics.
// ResetStats 重置事件统计。
...
// EnableEvent enables specific events (disables all others).
// EnableEvent 启用特定事件（禁用其他所有事件）。
// Call with no arguments to enable all events.
// 不传参数时启用所有事件。
...
// DisableEvent disables specific events.
// DisableEvent 禁用特定事件。
...
// RegisterWithConfig 使用自定义配置注册监听器。
...
// RegisterFuncWithConfig 使用自定义配置注册函数监听器。
...
// TriggerAsync triggers an event asynchronously and returns immediately.
// TriggerAsync 异步触发事件并立即返回。
func (m *Manager) TriggerAsync(data *EventData) {
	go m.Trigger(data)
}

// TriggerSync triggers an event synchronously and waits for all listeners.
// TriggerSync 同步触发事件并等待所有监听器完成。
...
// GetListenerIDs returns all listener IDs for a specific event.
// GetListenerIDs 获取指定事件的所有监听器 ID。
...
// GetAllEvents returns all events that have registered listeners.
// GetAllEvents 获取所有已注册监听器的事件。
...
// HasListeners checks if there are any listeners for a specific event.
