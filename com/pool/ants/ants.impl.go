...
type RenewPoolManager struct {
	...
	started   bool             // Indicates if pool manager-examples is running | 是否已启动
	closeOnce sync.Once        // Ensure Stop only executes once | 确保 Stop 只执行一次
}

// NewRenewPoolManagerWithDefaultConfig creates manager-examples with default config | 使用默认配置创建续期池管理器
...
// NewRenewPoolManagerWithConfig creates manager-examples with config | 使用配置创建续期池管理器
